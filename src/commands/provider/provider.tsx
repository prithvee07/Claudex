...
type Step =
  ...
  | { name: 'codex-check' }
  | { name: 'nvidia-key' }
  | { name: 'nvidia-base'; apiKey: string }
  | { name: 'nvidia-model'; apiKey: string; baseUrl: string | null }
...
export function buildCurrentProviderSummary(options?: {
  ...
  if (isEnvTruthy(processEnv.CLAUDE_CODE_USE_NVIDIA)) {
    return {
      providerLabel: 'NVIDIA NIM',
      modelLabel: getSafeDisplayValue(
        processEnv.NVIDIA_MODEL ?? 'moonshotai/kimi-k2-instruct',
        processEnv,
      ),
      endpointLabel: getSafeDisplayValue(
        processEnv.NVIDIA_BASE_URL ?? 'https://integrate.api.nvidia.com/v1',
        processEnv,
      ),
      savedProfileLabel,
    }
...
function ProviderChooser({
  ...
  const options: OptionWithDescription<ProviderChoice>[] = [
    ...
    },
    {
      label: 'NVIDIA NIM',
      value: 'nvidia',
      description: 'NVIDIA-hosted models: Kimi K2, Llama, DeepSeek, Qwen, Mistral and more',
    },
  ]
  ...
  if (summary.savedProfileLabel !== 'none') {
    options.push({
      ...
      value: 'clear',
      description: 'Remove .Claudex-profile.json and return to normal startup',
    })
  }
...
function ProviderWizard({ onDone }: { onDone: LocalJSXCommandOnDone }): React.ReactNode {
  ...
  // Resolve existing NVIDIA key from env or saved profile
  const existingNvidiaKey =
    process.env.NVIDIA_API_KEY ||
    (persisted?.profile === 'nvidia' ? persisted.env.NVIDIA_API_KEY : undefined) ||
    ''
          ...
          onChoose={value => {
            if (value === 'auto') {
              ...
              if (existingNvidiaKey) {
                // Already have a key — skip straight to base URL step
                setStep({ name: 'nvidia-base', apiKey: existingNvidiaKey })
              } else {
    ...
    case 'nvidia-key':
      return (
        <TextEntryDialog
          ...
          description={
            existingNvidiaKey
              ? 'Leave blank to reuse the current NVIDIA_API_KEY from this session. Get a free key at https://build.nvidia.com/'
              : 'Enter your NVIDIA API key. Get a free key at https://build.nvidia.com/'
          }
          initialValue=""
          placeholder="nvapi-..."
          mask="*"
          allowEmpty={Boolean(existingNvidiaKey)}
          validate={value => {
            const candidate = value.trim() || existingNvidiaKey
            return sanitizeApiKey(candidate)
              ? null
              : 'Enter a real API key.'
          }}
          onSubmit={value => {
            const apiKey = value.trim() || existingNvidiaKey
            setStep({ name: 'nvidia-base', apiKey })
          }}
          onCancel={() => setStep({ name: 'choose' })}
        />
      )
    ...
    case 'nvidia-base':
      return (
        <TextEntryDialog
          ...
          title="NVIDIA NIM setup"
          subtitle="Step 2 of 3"
          description="Optionally enter a base URL. Leave blank for the default NVIDIA NIM endpoint."
          initialValue={
            defaults.nvidiaBaseUrl === 'https://integrate.api.nvidia.com/v1'
              ? ''
          ...
          onCancel={() => setStep({ name: 'nvidia-key' })}
        />
      )
