...
export function buildNvidiaProfileEnv(options: {
  ...
  return {
    ...
    NVIDIA_BASE_URL: options.baseUrl ?? processEnv.NVIDIA_BASE_URL ?? 'https://integrate.api.nvidia.com/v1',
    NVIDIA_MODEL: options.model ?? processEnv.NVIDIA_MODEL ?? 'moonshotai/kimi-k2-instruct',
    NVIDIA_API_KEY: key,
  }
}
...
export function hasExplicitProviderSelection(
  ...
  return (
    ...
    processEnv.CLAUDE_CODE_USE_NVIDIA !== undefined
  )
}
...
export async function buildLaunchEnv(options: {
  ...
  if (options.profile === 'nvidia') {
    const nvidiaEnv: NodeJS.ProcessEnv = {
      ...processEnv,
      CLAUDE_CODE_USE_NVIDIA: '1',
    }

    delete nvidiaEnv.CLAUDE_CODE_USE_OPENAI
    delete nvidiaEnv.CLAUDE_CODE_USE_GEMINI
    delete nvidiaEnv.CLAUDE_CODE_USE_GITHUB

    nvidiaEnv.NVIDIA_BASE_URL =
      sanitizeProviderConfigValue(persistedEnv.NVIDIA_BASE_URL, persistedEnv) ||
      sanitizeProviderConfigValue(processEnv.NVIDIA_BASE_URL, processEnv) ||
      'https://integrate.api.nvidia.com/v1'
    nvidiaEnv.NVIDIA_MODEL =
      sanitizeProviderConfigValue(persistedEnv.NVIDIA_MODEL, persistedEnv) ||
      sanitizeProviderConfigValue(processEnv.NVIDIA_MODEL, processEnv) ||
      'moonshotai/kimi-k2-instruct'

    const nvidiaKey =
      sanitizeApiKey(persistedEnv.NVIDIA_API_KEY) ||
      sanitizeApiKey(processEnv.NVIDIA_API_KEY)
    if (nvidiaKey) {
      nvidiaEnv.NVIDIA_API_KEY = nvidiaKey
    } else {
      delete nvidiaEnv.NVIDIA_API_KEY
    }

    delete nvidiaEnv.OPENAI_BASE_URL
    delete nvidiaEnv.OPENAI_MODEL
    delete nvidiaEnv.OPENAI_API_KEY
    delete nvidiaEnv.GEMINI_API_KEY
    delete nvidiaEnv.GEMINI_MODEL
    delete nvidiaEnv.GEMINI_BASE_URL
    delete nvidiaEnv.GOOGLE_API_KEY
    delete nvidiaEnv.CODEX_API_KEY
    delete nvidiaEnv.CHATGPT_ACCOUNT_ID
    delete nvidiaEnv.CODEX_ACCOUNT_ID
...
export async function buildStartupEnvFromProfile(options?: {
  ...
  return buildLaunchEnv({
    ...
    goal:
      options?.goal ??
      normalizeRecommendationGoal(processEnv.CLAUDEX_PROFILE_GOAL),
    processEnv,
